  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-12">
            ...
            <table className="w-full text-left min-w-175 md:min-w-full">
              ...
              <thead>
                <tr className="bg-slate-900 text-white text-[10px] font-black uppercase tracking-[0.2em]">
                  ...
                  <th className="px-4 md:px-8 py-5">Region</th>
                  <th className="px-4 md:px-8 py-5 text-center">Invoices</th>
                  <th className="px-4 md:px-8 py-5 text-right">LTV (Billed)</th>
                    ...
                    >
                      <td className="px-4 md:px-8 py-6">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-blue-50 text-blue-600 rounded-xl shrink-0 flex items-center justify-center font-black text-sm">
                            {client.name?.charAt(0) ?? "C"}
                          </div>
                          <div className="min-w-0">
                            {" "}
                            <p className="font-bold text-slate-900 truncate max-w-30 md:max-w-full">
                              {client.name}
                            </p>
                            <p className="text-xs text-slate-400 truncate max-w-30 md:max-w-full">
                              {client.email}
                            </p>
                          </div>
                        </div>
                      ...
                      <td className="px-4 md:px-8 py-6 text-center text-sm font-bold text-slate-600">
                        {client.invoiceCount}
                      </td>
                      <td className="px-4 md:px-8 py-6 text-right font-black text-slate-900 whitespace-nowrap">
                      ...
                      <td className="px-4 md:px-8 py-6">
                        <div className="flex items-center justify-end gap-2">
                          <Link
                            ...
                            className="inline-flex items-center gap-2 px-3 md:px-4 py-2 bg-blue-600 text-white rounded-xl text-xs font-black hover:bg-slate-900 transition-all shadow-lg shadow-blue-100 whitespace-nowrap"
                          >
                            <ReceiptText size={14} />
      ...
      {archivedClients.length > 0 && (
        <details className="mt-12 group max-w-6xl mx-auto">
          ...
          <div className="mt-4 bg-white rounded-3xl md:rounded-4xl border border-slate-100 overflow-x-auto shadow-sm opacity-60 hover:opacity-100 transition-opacity">
            <table className="w-full text-left min-w-125 md:min-w-full">
              <tbody className="divide-y divide-slate-50">
                {archivedClients.map((client) => (
                  <tr key={client.id}>
                    <td className="px-4 md:px-8 py-4 text-sm font-bold text-slate-600">
                      {client.name}
                    </td>
                    <td className="px-4 md:px-8 py-4 text-xs text-slate-400">
                      {client.email}
                    </td>
